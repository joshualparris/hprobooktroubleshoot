...
param(
    ...
    [ValidateSet('doctor','status','sample','incident','collect','snapshot','experiment-start','experiment-end','experiment-status','help')]
...
Usage:
  ...
  .\windows-crash-doctor.ps1 snapshot -EvidencePath C:\Evidence\HPProBook-YYYYMMDD-HHMMSS
...
Commands:
  ...
  snapshot           Analyse a previously collected snapshot folder offline.
...
switch ($Command) {
    ...
    'snapshot' {
        if (-not $EvidencePath) {
            throw 'snapshot requires -EvidencePath.'
        }
        $snapshotCli = Join-Path $PSScriptRoot 'Invoke-CrashDoctor.ps1'
        if (-not (Test-Path -LiteralPath $snapshotCli)) {
            throw 'Offline snapshot analyser is missing from this installation.'
        }
        if ($OutputRoot) {
            & $snapshotCli -EvidencePath $EvidencePath -OutputDirectory $OutputRoot
        } else {
            & $snapshotCli -EvidencePath $EvidencePath
        }
    }
    'experiment-start' {
